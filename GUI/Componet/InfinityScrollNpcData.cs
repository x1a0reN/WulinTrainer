...
[RegisterInIl2Cpp]
internal class InfinityScrollNpcData : InfinityScrollBase
{
    private List<CharacterPoolData> _data;

    public List<CharacterPoolData> Data
    {
        get => _data;
        set
        {
    ...
    public InfinityScrollNpcData(IntPtr ptr) : base(ptr) { }
    ...
    protected override void UpdateItems(int startIndex)
    {
        for (int i = 0; i < _items.Count; i++)
        {
            ...
            itemRect.anchoredPosition = new Vector2(itemRect.anchoredPosition.x, -((float)row * _itemHeight) - (float)(startIndex / Columns) * _itemHeight - 50);
            var entry = _items[i].GetComponent<NpcEntry>();
            if (entry != null)
            {
