...
public class ItemPanel : MonoBehaviour
{
    ...
    private ItemType[][] _typeList = { 
        new ItemType[] { ItemType.Equip,
            ItemType.Equip_Weapon, ItemType.Equip_Armor, ItemType.Equip_Amulet,ItemType.Equip_Pet },
    ...
    private void LoadItemData()
    {
        ...
        foreach (var itemData in itemsConfig) {
            ...
            foreach (var group in _typeList) {
                ...
                else if ((itemData.Type & ItemType.Equip_Pet) == itemData.Type) {
                    _classifiedItems[ItemType.Equip].Add(itemData);
                    break;
                }
            }
        }
    ...
    private void UpdateItemList(int maintype, int subType = 0,string keywords = "")
    {
        ...
        (x.Type & _typeList[maintype][subType]) == x.Type && x.UName.Contains(keywords))
        .ToList();
            //为了处理宠物装备，并不属于Equip的子类型
            if (maintype == 0 && subType == 0)
            {
                var list = _classifiedItems[type].Where(x =>
            (x.Type & _typeList[maintype][4]) == x.Type && x.UName.Contains(keywords))
            .ToList();
