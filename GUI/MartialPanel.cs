...
internal class MartialPanel : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        for (int i = 0; i < _typeGroup.transform.childCount; i++) {
            ...
            toggle.onValueChanged.AddListener((bool value) => {
                if (value) {
                    ...
                    UpdateMartialList(type,g_keywords);
                }
            });
        }

        var search_keywords = transform.Find("MartialList/BottomBar/InputField").GetComponent<InputField>();
        search_keywords.onValueChanged.AddListener((string input) =>
        {
            //搜索记忆功能
            g_keywords = input;

            UpdateMartialList(_type,input);
        ...
        _infinityScroll.SpaceX = 6;
        _infinityScroll.SpaceY = 6;
    ...
    private void UpdateMartialList(MartialType type,string keywords = "")
    {
        if (keywords == "")
        {
            _type = type;
            ...
            _type = type;
            var temp = new List<KungfuData>();
            foreach (var item in _classifiedKungfus[type])
            {
                if (item.UName.Contains(keywords)) {
                    ToyBox.LogMessage(item.UName);
                    temp.Add(item);
                }
            }
            _infinityScroll.Data = temp;
        }
