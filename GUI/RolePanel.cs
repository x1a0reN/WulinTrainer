...
internal class RolePanel : MonoBehaviour
{
    ...
    readonly string[] additionRoleInfoKeys = {
        "hp", "mp", "bili", "tizhi", "minjie", "wuxing", "fuyuan","fanji","point"
    };
    ...
    readonly Dictionary<string, string> keyToLabelMap = new() {
        ...
        {"repexp", "名声经验"}, {"coin", "金币"},{"jianghulilian", "江湖历练"},{ "fanji","反击"}
     };
    ...
    private void Awake()
    {
        ...
        TraitEntryPrefab.SetActive(false);
        SetupRoleList();
        UpdateRoleInfo(PlayerTeamManager.Instance.GetTeamMemberByIndex(0));
    }
